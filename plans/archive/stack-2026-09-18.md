...
- Status 2026-09-18 19:11 (rota-ff): **closed.** The record is
  `plans/archive/principal-pages-2026-09-18.md` at commits fddfa52 and
  6d33f43. It lists all 32 pages by verb and kind, with the size of each in
  tokens, the reply and a judgement. Eleven facts carry to the frames
  below. The worst is that night 86 told the principal five claims were
  falsified where the Critic gave up after three attempts, which pushes
  frame 44. Actual against the 80k estimate, walls apart: 612k for the
  judgement pass of twelve agents, 823k for the refutation pass of twelve
  adversaries and one completeness critic, 1435k in agents (observed: the
  harness usage lines). Walls: none. The deviation is deliberate. Ultracode
  is on for this session, so the frame ran the adversarial pass that the
  80k line did not price, and that pass overturned 23 of the 32 first
  judgements. The extraction was not done by an agent, as the frame's price
  assumed. It is a script in the scratchpad, and the agents judged only.
