...
| 2026-09-13 | tipsAZ, tipsBA on the Titan: the misplaced-import door | | both froze in a Tester turn: a reply ran to the 8192-token cap with no call, the runner's cut-note path raised, the session died unrecorded and the same wake was re-dispatched for hours. Not a hang. Fixed (a8ce330, 2f79722): the cut goes back once, a failed session leaves a row, the attempt cap runs in the loop. No merge |
| 2026-09-13 | tipsBB on the Titan: the runner fixes | | amended, re-sliced, b3 built and one test passed; the Developer's batch_start quarantined three times over, the share written under a name that is not the criterion's surface `split_bill`, two questions to the Vision Keeper and never the unresolved exit (9B judgement, as tipsAX). Then a Tester turn timed out three times: at 27 tok/s a cap-length reply cannot finish in 300 s. Killed for the register re-record. No merge |
