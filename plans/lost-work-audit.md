...
|---|---|---|---|
| `plans/archive/completion-diary.md` (was COMPLETION.md:692) | The hello-world walk "becomes `S0-hello-world`, the permanent floor of the story tier". | No case, test or tier of that name exists. The S0 walks live in `probes/s0_walk.py` and the walk log. | Doc: the sentence is withdrawn in the documentation pass. The floor is the walk row in `tests/rota/walks.jsonl`. |
...
| `rota/LAWS.md:155-156` (law 4) | Boot hands the head-commit divergence to the woken role. | `boot.reconcile_worktrees` computes it. | It reached only the boot report line. | Done 2026-09-13: boot writes the real HEAD back to `batches.head_commit`, so every reader of the head (the verdict gate, the stray check, the churn door, the Developer's own source reads) sees the commit that exists. The law's sentence is rewritten to that. |
| `plans/archive/responsibility-allocation.md:40` | C2 verified on `batch_dep_facts`. | The table and `schedule_order`'s reader. | No op writes it; `batches.depend` is listed unbuilt in COMPLETION core item 10. | Known. `plans/principal-flow.md:375` records it as a dead end; the allocation grade is corrected in the documentation pass. |
| `plans/archive/responsibility-allocation.md:31` | B3 verified with the reference hash comparison. | The area-hash half is real. | The reference half is table 1, row 2. | Grade corrected with the drift item. |
