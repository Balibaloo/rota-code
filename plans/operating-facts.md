...
- The rota suite on WSL waited 22 minutes in ext4 journal waits with 16 workers idle. `tests/rota/conftest.py` sets `TMPDIR=/dev/shm` when that directory exists. The git fixture's guard follows `tempfile.gettempdir()`. `--basetemp` alone breaks the guard.
