...
def _seed(conn):
    ...
    conn.execute("INSERT INTO items (id, text, kind, provenance, approval, approval_ver, version) VALUES "
                 "('i1','each person pays an equal share of the total with tip','in_scope','decided','approved',1,1)")
    conn.execute("INSERT INTO item_statements (item_id, statement_id) VALUES ('i1','s1')")
    ...
    conn.execute("INSERT INTO criteria (id, ticket_id, text, term_refs) VALUES ('c1','tk1','each share is equal','[]')")
