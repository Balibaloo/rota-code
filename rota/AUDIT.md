...
| 8 | A run profile names the model and the endpoint the prompts go to. A project can ship one under `.rota/profiles/`. | `llm/profile.py` `places` | The project's profile is read first. `rota profile show` prints its endpoint. `rota onboard` says where prompts and code go. `profile check` says "remote" for an endpoint off this machine. Keys never sit in a profile: `key_env` names a variable, `~/.rota/keys.env` holds the value, restricted. | A keychain, later. |
