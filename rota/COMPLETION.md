# Completion: the core and the plan

What is owed for core and the plan to get there. What exists and where it
is lives in `MAP.md`. The walk diary and the closed tracks, 2026-08-28 to
2026-09-13, are archived in `plans/archive/completion-diary.md`; every
...
**Step 1, measured (2026-09-14, 15:16 to 16:13).** Five cases, five runs
each, one model per column, one load per column, thinking off on the
MoE, the same wakes:

| act | qwen3.5:9b | qwen2.5:14b | gemma-4-26B-A4B (llama.cpp) |
|---|---|---|---|
...
1. Publish the cassette database as the first release. The repository
   ...
   ```
   gh release create cassettes-20260914 --repo Balibaloo/rota-code --title cassettes-20260914 --latest --notes "cassettes 2026-09-14: 17745 cassettes, 135179 case runs" D:\repos\rota\.rota\release\cassettes.db.gz D:\repos\rota\.rota\release\cassettes.json
   ```
